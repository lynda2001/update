...
function CompanySignin() {
    ...
    const loginuser = async(e) => {
      ...
      if (email === "") {
        ...
        try {
          const response = await axios.post('http://localhost:5000/royecruit/login', {
            email,
            password,
          });
        ...
        <Grid container direction="row" alignItems="center" spacing={1}>
          <Grid item>
            <Button
              variant="outlined"
              startIcon={<Google />}
              sx={{marginBottom: 1, marginLeft:20, width: '300px', borderColor: '#db4437', color: 'white', backgroundColor:'#db4437' , marginTop: 2 , '&:hover': { backgroundColor: "transparent",  color: "#db4437", border: "1px solid #db4437"} }}
            >
              Google
            </Button>
          </Grid>
